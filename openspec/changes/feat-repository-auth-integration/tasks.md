...
- [x] **T-01** Add `applyClientsAuthFilter` in `clients.go` (~55 LOC). Defensive args copy; inject ` AND id = ?` before last `ORDER BY`/`LIMIT` for `RoleClient` (nil `ClientID` → `ErrForbidden`); admin/owner passthrough; staff/unknown → `*domain.SemanticError{ErrCodeForbidden}`.
...
- [x] **T-02** `RequireRole(admin, owner)` gate first in `Save`, `Create`, `Update`, `Delete`, `FindByPhone`. SQL unchanged.
  - **Dep**: none · **Est**: S · **Accept**: admin proceeds; others rejected, DB untouched. **Test**: role-matrix per method.

- [x] **T-03** `FindByID`: `RequireCaller` + `applyClientsAuthFilter`. Cross-tenant→`ErrNotFound`.
  - **Dep**: T-01 · **Est**: S · **Accept**: admin→any row; client→own row; cross-tenant→`ErrNotFound`; staff→`ErrForbidden`. **Test**: regexp `AND id = \?`.

- [x] **T-04** `SearchFTS`: `RequireCaller` + `applyClientsAuthFilter`. Clause before `ORDER BY bm25(f)`. Client no-match→empty slice, nil error.
...
- [x] **T-05** `GetOrCreate`: inline role switch — admin/owner unrestricted; client iff `phone == *caller.ClientID`; staff→`ErrForbidden`. Gate before validation.
...
- [x] **T-06** `RequireRole(admin, owner)` first in `Create`/`Delete`. SQL unchanged.
...
- [x] **T-07** `RequireCaller` (presence only) first in `Get`/`List`. No role check, no filter (hot-path guard).
  - **Dep**: none · **Est**: S · **Accept**: all 4 roles read; caller-less→`ErrUnauthenticated`. **Test**: 5-case table per REQ-BHE-AUTH-002.

## Phase 4: Test migration

- [x] **T-08** Migrate `clients_test.go` from `context.Background()` to role ctx helpers. Add `ErrUnauthenticated` cases. `ExpectationsWereMet` proves DB untouched.
  - **Dep**: T-02…T-05 · **Est**: M · **Accept**: `go test -race` green; no `context.Background()` on gated methods.

- [x] **T-09** Migrate `business_hours_exception_test.go` similarly. Add rejection cases for `Create`/`Delete`; all-role reads for `Get`/`List`.
  - **Dep**: T-06, T-07 · **Est**: M · **Accept**: hot-path scenario passes for all four roles.

## Phase 5: Verification

- [x] **T-10** Run `go fmt ./...`, `go vet ./...`, `golangci-lint run ./...`, `go build -o /dev/null ./...`, `go test -v -race ./...`. Confirm zero diff on `auth_filter.go`, `go.mod`, schema.
