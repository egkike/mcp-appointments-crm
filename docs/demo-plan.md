...
| 2026-09-26 | v0.6.1 | Pasos 1-4 + Paso 6 (from-zero) | ✅ smoke from-zero completo tras limpieza manual de la instalación v0.4.0 | Wizard TTY con datos demo unicode + deploy pineado `v0.6.1` literal al README (valida la doc de distribución); seed TUI del owner + opción 7 (X-Caller-Id correcto); sweep HTTP stateless 8/8 familias (domingo y fuera-de-horario rechazo semántico, FTS5, loyalty, mutaciones owner-only roundtrip) + prueba real por Telegram; un flake de modelo gateway-side (batching de 3 tools rechazado, retry OK, nunca llegó al server). Hallazgos: F-1 `bookings.id` admite NULL por SQL directo (falta CHECK); F-2 `business_hours`/`accepted_payment_methods` salen como JSON stringificado (ergonomía LLM); F-3 sin cross-validation negocio↔profesional en setup (runtime SÍ protege por intersección, verificado con servidor sandbox: 17:30 disponible, 18:00 rechazado) |
| 2026-09-26 | v0.6.1 | Paso 2 re-run (idempotencia) | ✅ deploy reproducido idéntico sobre instalación viva: binario swap atómico mismo hash + restart del servicio, DB/caller-id/.env/setup JSONs/entrada Hermes todos intactos | Verificación pre-doc del claim "update = re-run Paso 2 con tag más nuevo" (fotografía pre/post con hashes); nota de updating agregada al README; es el patrón de prueba para el upgrade-aware deploy (dos corridas consecutivas) |
...
- **Gaps Fase N descubiertos en la corrida 2026-09-10**:
  - ✅ **RESUELTO (2026-09-11)** — No existe flujo wizard→DB: el binario no tiene `--seed`/`--setup` y los 3 JSONs de setup no tienen consumidor en el deploy; el seed quedó como SQL manual (Paso 5). Definir el consumidor de los JSONs (Fase N). Cerrado por `feat-setup-import` (PRs #72/#73/#74, issue #71): `config.SeedOnBoot` consume los 3 JSONs en el primer arranque y siembra `business_profile`, `professionals`, `schedules` y `services`; los arranques siguientes son no-op. No crea fila en `accounts` (el alta del owner sigue siendo la TUI, §7).
