...
- [ ] **O2**: Alcanzar un tiempo de instalación en una VPS Ubuntu limpia (sólo con `curl` y `bash`) inferior a 5 minutos, medido desde el wizard interactivo (que requiere una TTY real y no puede pipearse) más el deploy pineado `curl … | bash -s -- --version <tag>` hasta el log "Servidor MCP Activo".
...
- Script `install.sh` que descarga el binario, lo registra como servicio del SO e imprime al final la invocación de `backup.sh` como recordatorio (sin sugerir ningún scheduler; el scheduling queda a criterio del operador).
...
- **Binario nativo en Go 1.26.8** con `modernc.org/sqlite` (pure Go, sin CGo, sin capas de contenedor). Se distribuye como binario único cross-compiled para 5 plataformas. Binario corre como **user-level service** (sin root, sin `appuser` dedicado) bajo el usuario que invoca `install.sh`.
...
- `internal/domain/` — capa de dominio (Fase 1b): `entity/` (entidades con comportamiento), `repository/` (interfaces), `service/` (reglas de negocio, p.ej. la cadena de 5 pasos de `check_availability` en §3.7.13) y `errors.go` (errores de dominio). Todos implementados y en producción. Dependencia cero de infraestructura.
...
- `internal/model/` — structs de dominio originales. **Eliminado** en Fase 1b (commit `8c90861`, "P3.4 infra cleanup"): consumidores migrados a `internal/domain/entity/`.
- `internal/application/` — capa de aplicación (Fase 1b): `usecase/` (casos de uso, p.ej. `check_availability.go`) y `dto/` (objetos de transferencia). Los use cases se inyectan en los handlers MCP (Fase 2).
- `internal/auth/` — capa de autorización (Fase 0): schema `accounts`, repos y middleware `AuthMiddleware`. En producción.
...
| **Logs** | Journal del usuario: `journalctl --user -u mcp-appointments-crm` (el unit no escribe archivo de log) | `~/Library/Logs/MCP Appointments CRM/mcp-server.out.log` + `mcp-server.err.log` | Sólo diseño objetivo (`%LOCALAPPDATA%\MCP Appointments CRM\Logs\`) |
...
>
> **Convenciones XDG**: si `XDG_DATA_HOME`, `XDG_CONFIG_HOME` o `XDG_STATE_HOME` están definidas, se respetan como base de los paths de data/config/logs. En Linux el servicio escribe sus logs en el journal (no genera archivo de log), así que `XDG_STATE_HOME` no se usa para los logs del servicio.
>
...
6. ✅ **Deuda de auth CERRADA (2026-08-23)** — `internal/repository/clients.go` (8 métodos) y `business_hours_exception.go` (4 métodos) cableados con `auth.Caller` en `feat-repository-auth-integration` (PR #50, `5aca28d`, 7 REQ, 22 escenarios, JD 2/2 PASS, re-verify PASS 88.5%). Todos los repos de PR 2 ya tienen wiring; no queda deuda pendiente para Fase 3. Los tools MCP de Fase 2 consumen use cases de `internal/application/` (filtrado por caller), no los repos directamente.
...
**Nota sobre la integración de auth en repos de PR 2:** los repos de PR 1+2 fueron escritos antes de que `feat-authorization` estuviera mergeado. `services.go` se cableó en Fase 1; `clients.go` y `business_hours_exception.go` se cerraron el 2026-08-23 en `feat-repository-auth-integration` (PR #50). Todos los repos ya tienen `auth.Caller` wiring; Fase 3 ya no tiene prerequisitos pendientes.
...
- **Prioridad**: Must
- **Estado**: `get_business_profile()` implementado en Fase 2 (PR #47, `internal/mcp/tools_profile.go` + use case `GetBusinessProfile`). `update_business_profile` implementado en Fase N (owner-only, merge parcial: solo se aplican los campos enviados; ver [ADR-0015](../architecture/0015-hermes-operational-maintenance.md)).
...
- **Criterios de Aceptación**:
  ...
  - [ ] Dado que el script se ejecuta sin los archivos JSON de `setup/`, cuando el sistema valida los prerrequisitos, entonces imprime `Error: faltan archivos de setup: setup_business.json setup_staff.json setup_services.json` y termina con exit code 1 sin instalar el binario ni registrar el servicio.
  - [ ] Dado que el script terminó exitosamente, cuando el operador revisa la salida, entonces encuentra la invocación de `backup.sh` lista para replicar, pero **no** un snippet de `crontab` ni de ningún scheduler: `install.sh` no configura ni sugiere ninguna tarea periódica (el scheduling queda a criterio del operador).
  - [ ] Dado que el operador revisa la salida de `install.sh`, entonces el log final incluye un bloque estático en español titulado `Herramientas recomendadas:` que nombra herramientas opcionales (ej. `jq`, `sqlite3`, `ufw`, `hermes doctor`) **sin detectar su presencia ni imprimir comandos de instalación**, y sin ejecutar ninguna instalación (ver [ADR-0005](../architecture/0005-optional-external-tools.md)).
...
| Disponibilidad | El servicio debe reiniciarse automáticamente ante crash | Unit de systemd con `Restart=on-failure` + `RestartSec=5` (equivalente launchd `KeepAlive=true`) |
...
| Seguridad | Puerto expuesto solo en loopback; bind y puerto configurables vía env vars + `.env` | Doble capa: bind a `127.0.0.1` (IPv4) o `::1` (IPv6) (default, configurable vía `MCP_BIND` con validación de loopback 127.0.0.0/8 o ::1) + validación de loopback al arranque del propio binario (el systemd unit no incluye `IPAddressAllow`; ver ADR-0007). Puerto default `3000` configurable vía `MCP_PORT`. Precedencia: env vars > `~/.config/mcp-appointments-crm/.env` > defaults. Ver ADR-0007. |
...
- **Backend**: Go 1.26.8 (binario `mcp-server`)
...
**Estado al 2026-08-09 (Fase 1 CERRADA):**

- ✅ **PR 1 + PR 2** (foundation: schema 10-tablas + 8 models + sentinels + FTS5 integration test + 4 repos simples) — mergeados en main como parte de PR #7 (commit `2c1ffd8`).
...
- ✅ **PR 3b** — `CheckAvailability` implementado como **use case** en Fase 1b: `internal/application/usecase/check_availability.go`.

**Stats finales de Fase 1** (medidas al cierre, 2026-08-09):
- 10 repos implementados (todos, incluido `CheckAvailability` como use case en `internal/application/usecase/check_availability.go`)
- Cobertura real medida en el cierre: `internal/repository` 87.6%, `internal/domain` 100%, `internal/application/usecase` 83.4%, `internal/auth` 86.2%, `internal/domain/entity` 97.0%, `internal/domain/service` 88.3%
...
- Impresión al final de `install.sh` de la invocación de `backup.sh` (sin auto-configurar ni sugerir ningún scheduler; el operador decide cómo agendarlo)
- Lista informativa al final del log (`Herramientas recomendadas:`) que nombra herramientas opcionales (ej. `jq`, `sqlite3`, `ufw`, `hermes doctor`) sin detectar su presencia ni imprimir comandos de instalación; **nunca ejecuta la instalación** (ver [ADR-0005](../architecture/0005-optional-external-tools.md))
...
- **Bot de messenger del negocio — WhatsApp con identidad por emisor (Fase 2+; decisión de producto 2026-09-18, refinada 2026-09-26)** — el canal de producción es el número de WhatsApp del negocio: el bot lee el `from` de cada mensaje y lo inyecta como `X-Caller-Id` (per-sender identity, PRD §3.8.9), y el RBAC existente aplica solo el rol correcto: el Owner (su phone real en `accounts` vía Transfer Ownership) puede pedir cambios al negocio/servicios/horarios; cualquier phone sin fila en `accounts` cae a rol client (self-service de sus reservas). **La sesión de WhatsApp de Hermes se abre SIEMPRE con el número del negocio (`business_profile.messenger_id`)** — los emisores se resuelven por su JID entrante contra la cadena accounts → `clients.phone` → desconocido, idéntica en ambos escenarios de conexión:
  - **Escenario A — owner n° ≠ n° negocio**: el owner escribe al número del negocio desde su número personal; su JID entrante resuelve como owner naturalmente (caso simétrico y limpio).
  - **Escenario B — owner n° = n° negocio (decisión de producto 2026-09-26: SOPORTADO)** — negocios mono-línea que solo tienen un número: el owner usa el **self-chat** de WhatsApp ("Message yourself", JID propio) como canal de mantenimiento owner. El spec debe tratar el self-chat como sender válido con rol resuelto por `accounts`, y declarar explícitamente este escenario (la sesión es del mismo número que la cuenta del owner).
  **Contrato de ubicación (decisión de producto 2026-09-26)** — el owner carga/corrige la ubicación del negocio por chat en sus dos formas naturales, y el cliente puede recibirla de vuelta:
  - **Map → texto**: el owner comparte el **pin/attachment de ubicación** de WhatsApp; el gateway/agent de Hermes extrae lat/long del mensaje y llama `update_business_profile` (nuestro MCP solo recibe números normalizados por diseño de capas — el parsing de attachments es responsabilidad de la plataforma mensajería). Fallback textual si el attachment no se puede parsear: pedirle las coordenadas al owner por chat.
  - **Texto → mapa**: las respuestas al cliente derivan de coords: `get_business_profile` debe exponer además un **`maps_url` derivado** (`https://maps.google.com/?q=lat,long`) junto a lat/long, listo para que el bot envíe el link/ubicación sin aritmética propia.
...
- ✅ **Tools Hermes de mantenimiento de datos operativos — ENTREGADO (Fase N)** (perfil, servicios, profesionales, horarios/agendas; ver [ADR-0015](../architecture/0015-hermes-operational-maintenance.md)) — 8 tools MCP owner-only (`update_business_profile`, `create_service`, `update_service`, `delete_service`, `create_professional`, `update_professional`, `upsert_schedule`, `delete_schedule`); la superficie MCP pasa de 11 a 19 tools. Hermes ya puede modificar los datos que el install siembra en el primer arranque, así que el mantenimiento de perfil/servicios/profesionales/horarios ya no requiere SQL manual. RBAC owner para el MVP; admin parcial queda diferido. Alcance negativo vigente: no gestiona cuentas (eso es la TUI, [ADR-0016](../architecture/0016-admin-tui-scope.md)).
...
**Orden de ejecución sugerido** (por dependencia): 1º TUI identidad + owner seed ✅ (2026-09-16); 2º tools Hermes de mantenimiento ✅ (2026-09-17); 3º GoReleaser + releases por CI ✅ (2026-09-18); 4º TUI Configurar Hermes ✅ (2026-09-24); 5º smoke from-zero en VM contra v0.6.1 ✅ (2026-09-26, wizard + deploy pineado por README + seed TUI + opción 7 + sweep 8 familias tools + Telegram real; bitácora en demo-plan con hallazgos F-1/F-2/F-3) — pendiente: transición a producción, bot WhatsApp per-sender (con escenarios A/B + self-chat, ver arriba) y `scripts/uninstall.sh`.
